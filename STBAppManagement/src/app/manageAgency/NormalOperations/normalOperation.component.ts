...
@Component({
  ...
  templateUrl: './normalOperation.component.html',
  styleUrls: ['./normalOperation.component.scss']
})
export class NormalOperationComponent {
  ...
  constructor(private modal: NgbModal, 
              ...
              private norOperationService : NormalOperationService,
  ...
  getData(view,viewDate){
    ...
    this.norOperationService.GetSecureOperationsByAgency(this.Employe.agence.codeAgence,date.getDate(),(date.getMonth().valueOf()+1),date.getUTCFullYear(),view,this.getWeekNumber(date)).subscribe(
      
      res => {
        this.normalOperations = res;
...
getDataOperations(){
  this.norOperationService.GetStatisticalOp(this.Employe.agence.codeAgence).subscribe(
    res => {
      this.statOp = res;
