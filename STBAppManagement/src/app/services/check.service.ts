...
export class CheckService {
    ...
    //-- METHODES
    GetSecureDataCheckByAgency(IdAgence,day,month,year,TypeDate,CurrentWeek) {
        return this.httpS.get(`http://localhost:44365/api/GetSecureDataCheckByAgency/`+IdAgence+`/`+day+`/`+month+'/'+year+'/'+TypeDate+'/'+CurrentWeek)
        .map(function (res) { return res.json(); });
       }
       
    GetStatisticalCheck(IdAgence) {
        return this.httpS.get(`http://localhost:44365/api/GetStatisticalCheck/`+IdAgence)
        .map(function (res) { return res.json(); });
    }
