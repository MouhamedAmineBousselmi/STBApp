...
export class NormalOperationService {
    ...
    //-- METHODES
       ...
       GetSecureOperationsByAgency(IdAgence,day,month,year,TypeDate,CurrentWeek) {
        return this.httpS.get(`http://localhost:44365/api/GetSecureDataOperationsByAgency/`+IdAgence+`/`+day+`/`+month+'/'+year+'/'+TypeDate+'/'+CurrentWeek)
        .map(function (res) { return res.json(); });
       }
       
       GetStatisticalOp(IdAgence) {
        return this.httpS.get(`http://localhost:44365/api/GetSecureDataOperationsByAgency/`+IdAgence)
        .map(function (res) { return res.json(); });
       }
