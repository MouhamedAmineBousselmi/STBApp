...
export class ChangeService {
    ...
    //-- CONVERTING CURRENCY
    ConvertisseurDevise(from,to) {
        return this.httpS.get(`https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=`+from+`&to_currency=`+to+`&apikey=LAO7NV8ZK5ME1WI2`)
        .map(function (res) { return res.json(); });
    }
    //-- END CONVERTING CURRENCY

    //-- ADDING CURRENCY OPERATION
    AddDeviseOperation(body){
        return this.authPost$("http://localhost:44365/api/AchatVenteDevise",body);
    }
    //-- END ADDING CURRENCY OPERATION

    //-- Get List Changes
    GetListChanges(idEmploye){
        return this.authGet$("http://localhost:44365/api/GetListChangesByEmploye/"+idEmploye);
    }
    //-- END ADDING CURRENCY OPERATION
